...
function canManageTimeSlot(slot) {
  ...
  return (
    user.role === "admin" ||
    (user.role === "advisor" && Number(slot.advisor_id) === Number(user.id))
  );
}
...
async function login(event) {
  ...
  try {
    ...
    if (!user) {
      setMessage(
        "login-message",
        "Không tải được thông tin tài khoản sau đăng nhập.",
        true,
      );
      return;
    }
...
async function loadProfile() {
  ...
  if (
    !requireAuth({ guardId: "dashboard-guard", contentId: "dashboard-content" })
  ) {
    return;
  }
...
async function submitProfileForm(event) {
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        "profile-message",
        getErrorMessage(data, "Cập nhật họ tên thất bại"),
        true,
      );
      return;
    }
...
function renderSelectOptions(services, timeslots) {
  ...
  serviceSelect.innerHTML = activeServices.length
    ? `<option value="">Chọn loại tư vấn</option>${activeServices
        .map(
          (service) => `
  ...
  timeslotSelect.innerHTML = availableSlots.length
    ? `<option value="">Chọn khung giờ</option>${availableSlots
        .map(
          (slot) => `
...
function resetServiceForm() {
  ...
  document.getElementById("service-is-active") &&
    (document.getElementById("service-is-active").value = "true");
...
function openServiceModal(service = null) {
  ...
  if (service) {
    ...
    document.getElementById("service-description").value =
      service.description || "";
    document.getElementById("service-is-active").value = service.is_active
      ? "true"
      : "false";
  }
...
async function submitServiceForm(event) {
  ...
  const payload = {
    ...
    description:
      document.getElementById("service-description").value.trim() || null,
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        "service-message",
        getErrorMessage(data, "Lưu danh mục thất bại"),
        true,
      );
      return;
    }
...
async function deleteService(serviceId) {
  if (
    !window.confirm(
      "Bạn có chắc chắn muốn xóa hoặc ngưng hoạt động danh mục này không?",
    )
  ) {
    return;
  }
  try {
    ...
    if (!response.ok) {
      setMessage(
        ...
        getErrorMessage(data, "Xóa danh mục thất bại"),
        true,
      );
      return;
    }
...
function filterTimeSlots() {
  ...
  const user = getCurrentUser();
  const scope =
    document.getElementById("timeslot-filter-scope")?.value || "mine";
  const advisorKeyword = normalizeText(
    document.getElementById("timeslot-filter-advisor")?.value,
  );
  ...
  if (role === "advisor" && scope === "mine") {
    filtered = filtered.filter(
      (slot) => Number(slot.advisor_id) === Number(user?.id),
    );
  }
  if (role !== "advisor" && advisorKeyword) {
    filtered = filtered.filter((slot) => {
      const advisorName = normalizeText(
        slot.advisor_name || `Cố vấn #${slot.advisor_id}`,
      );
      const advisorId = String(slot.advisor_id);
      const compactKeyword = advisorKeyword.replace("#", "");
      return (
        advisorName.includes(advisorKeyword) ||
        advisorId.includes(compactKeyword)
      );
      //      return hasExactWord(slot.advisor_name || `Cố vấn #${slot.advisor_id}`, advisorKeyword)
      //      || advisorId === compactKeyword;
    });
  }
  if (status) {
...
async function prepareTimeslotAdvisorSelect(selectedAdvisorId = "") {
  ...
  if (user.role === "admin") {
    ...
    try {
      ...
      advisorSelect.innerHTML =
        Array.isArray(advisors) && advisors.length
          ? `<option value="">Chọn cố vấn phụ trách</option>${advisors
              .map(
                (advisor) => `
...
function resetTimeslotForm() {
  const user = getCurrentUser();
  document.getElementById("timeslot-id") &&
    (document.getElementById("timeslot-id").value = "");
  document.getElementById("timeslot-date") &&
    (document.getElementById("timeslot-date").value = "");
  document.getElementById("timeslot-start") &&
    (document.getElementById("timeslot-start").value = "");
  document.getElementById("timeslot-end") &&
    (document.getElementById("timeslot-end").value = "");
  const advisorSelect = document.getElementById("timeslot-advisor-id");
  if (advisorSelect) {
    if (user?.role === "advisor") {
      advisorSelect.value = String(user.id);
...
async function openTimeSlotModal(timeslot = null) {
  ...
  if (timeslot) {
    ...
    document.getElementById("timeslot-start").value = String(
      timeslot.start_time,
    ).slice(0, 5);
    document.getElementById("timeslot-end").value = String(
      timeslot.end_time,
    ).slice(0, 5);
  }
...
async function submitTimeslotForm(event) {
  ...
  if (!timeslotId) {
    payload.advisor_id = Number(
      document.getElementById("timeslot-advisor-id").value,
    );
  }
  if (
    !payload.slot_date ||
    !payload.start_time ||
    !payload.end_time ||
    (!timeslotId && !payload.advisor_id)
  ) {
    setMessage(
      "timeslot-message",
      "Vui lòng nhập đầy đủ thông tin khung giờ.",
      true,
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        "timeslot-message",
        getErrorMessage(data, "Lưu khung giờ thất bại"),
        true,
      );
      return;
    }
...
async function deleteTimeSlot(timeslotId) {
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        "timeslot-page-message",
        getErrorMessage(data, "Xóa khung giờ thất bại"),
        true,
      );
      return;
    }
    setMessage(
      "timeslot-page-message",
      data.message || "Xóa khung giờ thành công.",
    );
    await loadTimeSlots();
  } catch (error) {
...
async function loadMyAppointments() {
  if (
    !requireRole(["student"], {
      guardId: "my-appointments-guard",
      contentId: "my-appointments-content",
    })
  ) {
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      setEmpty(
        "my-appointments-result",
        getErrorMessage(data, "Không tải được lịch hẹn"),
      );
      return;
    }
...
async function loadBookingData(showSuccessMessage = false) {
  if (
    !requireRole(["student"], {
      guardId: "booking-guard",
      contentId: "booking-content",
    })
  ) {
    return;
  }
  ...
  try {
    ...
    if (!servicesResponse.ok) {
      ...
      setMessage(
        "booking-message",
        getErrorMessage(services, "Bạn cần đăng nhập trước"),
        true,
      );
      return;
    }
    if (!timeslotsResponse.ok) {
      ...
      setMessage(
        "booking-message",
        getErrorMessage(timeslots, "Không tải được khung giờ tư vấn"),
        true,
      );
      return;
    }
...
async function createAppointment(event) {
  ...
  if (!serviceSelect.value || !timeslotSelect.value) {
    setMessage(
      "booking-message",
      "Vui lòng chọn loại tư vấn và khung giờ.",
      true,
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        "booking-message",
        getErrorMessage(data, "Đặt lịch thất bại"),
        true,
      );
      return;
    }
    ...
    setMessage(
      "booking-message",
      `Đặt lịch thành công. Mã lịch hẹn: #${data.id}`,
    );
    event.target.reset();
...
async function cancelAppointmentWithReason(
  appointmentId,
  messageTargetId,
  reloadCallback,
) {
  ...
  try {
    ...
    if (!response.ok) {
      setMessage(
        messageTargetId,
        getErrorMessage(data, "Hủy lịch thất bại"),
        true,
      );
      return;
    }
...
async function loadAdvisorAppointments() {
  if (
    !requireRole(["advisor", "admin"], {
      guardId: "advisor-appointments-guard",
      contentId: "advisor-appointments-content",
    })
  ) {
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      setEmpty(
        "advisor-appointments-result",
        getErrorMessage(data, "Không tải được danh sách lịch hẹn"),
      );
      return;
    }
...
async function updateAppointmentStatus(appointmentId, action) {
  ...
  try {
    const response = await fetch(
      `${API_BASE}/appointments/${appointmentId}/${endpoint}`,
      {
        method: "PUT",
        headers: getAuthHeaders(),
      },
    ...
    if (!response.ok) {
      setMessage(
        "advisor-appointments-message",
        getErrorMessage(data, "Thao tác thất bại"),
        true,
      );
      return;
    }
...
function bindMyAppointmentActions() {
  ...
  target.addEventListener("click", (event) => {
    const button = event.target.closest(
      "[data-my-appointment-action='cancel']",
    );
    if (!button) {
      return;
    }
...
async function loadAdminUsers() {
  if (
    !requireRole(["admin"], {
      guardId: "admin-users-guard",
      contentId: "admin-users-content",
    })
  ) {
...
function filterUsers() {
  const keyword = normalizeText(
    document.getElementById("user-filter-keyword")?.value,
  );
  ...
  if (keyword) {
    filtered = filtered.filter(
      (user) =>
        ...
        // hasExactWord(user.full_name, keyword)
        //    || hasExactWord(user.email, keyword)
    );
  }
  if (role) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("load-profile-btn")
    ?.addEventListener("click", loadProfile);
  document
    .getElementById("profile-form")
    ?.addEventListener("submit", submitProfileForm);
  document
    .getElementById("load-services-btn")
    ?.addEventListener("click", loadServices);
  document
    .getElementById("open-service-modal-btn")
    ?.addEventListener("click", () => openServiceModal());
  document
    .getElementById("load-timeslots-btn")
    ?.addEventListener("click", loadTimeSlots);
  document
    .getElementById("timeslot-filter-scope")
    ?.addEventListener("change", filterTimeSlots);
  document
    .getElementById("timeslot-filter-advisor")
    ?.addEventListener("input", filterTimeSlots);
  document
    .getElementById("timeslot-filter-status")
    ?.addEventListener("change", filterTimeSlots);
  document
    .getElementById("timeslot-filter-date")
    ?.addEventListener("change", filterTimeSlots);
  document
    .getElementById("reset-timeslot-filters-btn")
    ?.addEventListener("click", resetTimeSlotFilters);
  document
    .getElementById("open-timeslot-modal-btn")
    ?.addEventListener("click", () => openTimeSlotModal());
  document
    .getElementById("load-my-appointments-btn")
    ?.addEventListener("click", loadMyAppointments);
  document
    .getElementById("load-booking-data-btn")
    ?.addEventListener("click", () => loadBookingData(true));
  document
    .getElementById("booking-form")
    ?.addEventListener("submit", createAppointment);
  document
    .getElementById("load-advisor-appointments-btn")
    ?.addEventListener("click", loadAdvisorAppointments);
  document
    .getElementById("service-form")
    ?.addEventListener("submit", submitServiceForm);
  document
    .getElementById("timeslot-form")
    ?.addEventListener("submit", submitTimeslotForm);
  document
    .getElementById("load-admin-users-btn")
    ?.addEventListener("click", loadAdminUsers);
  document
    .getElementById("user-filter-keyword")
    ?.addEventListener("input", filterUsers);
  document
    .getElementById("user-filter-role")
    ?.addEventListener("change", filterUsers);
  document
    .getElementById("user-filter-status")
    ?.addEventListener("change", filterUsers);
  document
    .getElementById("reset-user-filters-btn")
    ?.addEventListener("click", resetUserFilters);
